...
D<-read_csv(file.path(PATH_TO_DATA,"/Data/raw/bdm_raw.csv"))
...
G<-S
G$boxes<-100; G$hasnoise<-TRUE; G$session<-5;G$imistake<-FALSE
G[G$listName=='LA10',]$listName<-'A10'
G[G$listName=='LA15',]$listName<-'A15'
G[G$listName=='r_LA10',]$listName<-'r_A10'
# write.table(S,file.path(PATH_TO_DATA,"/Data/parsed/bdm_parsed.csv"),sep=",",row.names=FALSE)

G <- G %>%mutate(across(c(a_rows, errors,finalPay,cuM,cuL,precisionL,precisionM,attnBoxL,attnBoxM,attnPayL,attnPayM), ~ as.integer(.)))


# T<-read.csv('~/Library/CloudStorage/Dropbox/@Projects/simplicity equivalents/Replication/Data/parsed/bdm_parsed.csv')
