...
D<-read_csv(file.path(PATH_TO_DATA,"/Data/raw/error1_raw.csv"))
...
A<-S
A$boxes<-100; A$hasnoise<-TRUE; A$session<-1; A$imistake<-TRUE

# write.table(S,file.path(PATH_TO_DATA,"/Data/parsed/error1_parsed.csv"),sep=",",row.names=FALSE)
